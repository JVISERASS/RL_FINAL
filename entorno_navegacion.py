...
class Navegacion(gym.Env):
    ...
    def reset(self):
        ...
        self.steps = 0
        self.max_steps = 500  # Reducido para evitar episodios muy largos
    ...
    def step(self, action):
        ...
        # Reward calculation
        if self.collision:
            reward = -200  # Penalización fuerte por colisión
        elif self.target:
            reward = 200   # Recompensa alta por llegar al objetivo
        else:
            reward = -1    # Pequeña penalización por paso para incentivar eficiencia
        ################################
